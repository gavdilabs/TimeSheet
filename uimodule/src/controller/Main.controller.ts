...
export default class Main extends BaseController {
  ...
  async setTimeSheetCharts() {
    ...
    (this.getView().byId("hoursLabel") as Label).setText(
      Math.round(totalWorkedHours) + "/" + totalHours + " hours"
    );
    (this.getView().byId("hoursProgress") as ProgressIndicator).setPercentValue(Math.round(totalWorkedHours / totalHours * 100));
    (this.getView().byId("hoursProgress") as ProgressIndicator).setDisplayValue(Math.round(totalWorkedHours / totalHours * 100) + "%");
    const hoursState = Math.round(totalWorkedHours / totalHours * 100) < 100 ? "None" : (Math.round(totalWorkedHours / totalHours * 100) == 100 ? "Success" : "Error");
    (this.getView().byId("hoursProgress") as ProgressIndicator).setState(hoursState);

    (this.getView().byId("daysProgress") as ProgressIndicator).setPercentValue(Math.round(workHoursMap.size / workScheduleMap.size * 100));
    (this.getView().byId("daysProgress") as ProgressIndicator).setDisplayValue(Math.round(workHoursMap.size / workScheduleMap.size * 100) + "%");
    const daysState = Math.round(workHoursMap.size / workScheduleMap.size * 100) < 100 ? "None" : (Math.round(workHoursMap.size / workScheduleMap.size * 100) == 100 ? "Success" : "Error");
